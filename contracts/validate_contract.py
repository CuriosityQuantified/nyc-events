...
REQUIRED_FACETS = {"borough", "category", "registration", "location"}
FORBIDDEN_KEYS = {"occurrence", "venue", "preference"}
CSV_FACT_COLUMNS = {
    "title": "title",
...
STATED_TEXT_FACTS = ("title", "description", "official_event_url", "location_id", "location_name")
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            if key == "$ref" and (not isinstance(child, str) or not child.startswith("#/")):
                raise ValueError(f"only local OpenAPI references are allowed: {child!r}")
...
def validate_operation(spec: dict[str, Any], route: str, operation: dict[str, Any]) -> Path:
    ...
    Draft202012Validator(root_schema, format_checker=FormatChecker()).validate(load_json(golden_path))
    return golden_path
...
BOROUGHS = {"M": "Manhattan", "B": "Brooklyn", "Q": "Queens", "R": "Staten Island", "X": "Bronx"}
...
def parsed_coordinates(raw: str) -> list[dict[str, float]]:
    ...
    for pair in raw.split(";"):
        ...
        coordinates.append({"latitude": float(latitude.strip()), "longitude": float(longitude.strip())})
    return coordinates


def local_iso(raw: str) -> str:
    parsed = datetime.strptime(raw, "%Y-%m-%d %H:%M:%S").replace(tzinfo=ZoneInfo("America/New_York"))
    return parsed.isoformat()


def verify_csv_reference(events: list[dict[str, Any]]) -> None:
    if len(CSV_PATHS) != 1:
        raise ValueError(f"expected one committed NYC Parks CSV reference, found {len(CSV_PATHS)}")
    ...
    for event in events:
        ...
        if row is None:
            raise ValueError(f"golden Event guid is absent from CSV reference: {event['guid']}")
        ...
        for fact_name in STATED_TEXT_FACTS:
            if event[fact_name]["value"] != event[fact_name]["raw"]:
                raise ValueError(f"{event['guid']} {fact_name}.value drifted from its Stated raw source text")

        expected_categories = [category.strip() for category in row["categories"].split("|") if category.strip()]
        if event["categories"]["value"] != expected_categories:
            raise ValueError(f"{event['guid']} categories.value drifted from the CSV reference")
        if event["coordinates"]["value"] != parsed_coordinates(row["coordinates"]):
            raise ValueError(f"{event['guid']} coordinates.value drifted from the CSV reference")

        expected_derived = {
            "start_date": datetime.strptime(row["startdate"], "%m/%d/%Y").date().isoformat(),
            "end_date": datetime.strptime(row["enddate"], "%m/%d/%Y").date().isoformat(),
        ...
        for fact_name, expected in expected_derived.items():
            if event[fact_name]["value"] != expected or event[fact_name]["provenance"] != "Derived":
                raise ValueError(f"{event['guid']} {fact_name} is not the expected Derived value")
        ...
        if (registration["value"], registration["provenance"]) != (
            ...
            raise ValueError(f"{event['guid']} registration_status drifted from source registration text")
        if event["registration_description"]["value"] != (row["registration_description"] or None):
            raise ValueError(f"{event['guid']} registration_description.value drifted from the CSV reference")

    freshness = load_json(ROOT / "golden/freshness.json")
    if freshness["snapshot_row_count"]["value"] != len(rows):
        raise ValueError("freshness snapshot_row_count does not match the committed CSV reference")
...
def main() -> None:
    ...
    if not REQUIRED_FACETS <= list_parameters | referenced_parameters:
        raise ValueError("Event listing is missing one or more required Facet parameters")
    ...
    if provenance_values != expected:
        raise ValueError(f"goldens must demonstrate all provenance states: {provenance_values!r}")

    print(f"validated {len(golden_paths)} golden responses against OpenAPI; CSV reference and vocabulary checks passed")
