    ...
    try {
      ...
      if (replace) {
        const [eventsResult, freshnessResult] = await Promise.allSettled([
          fetch(`/api/events?page=${targetPage}`, { cache: "no-store" }),
        ...
        eventsResponse = await fetch(`/api/events?page=${targetPage}`, {
          cache: "no-store",
        });
        if (requestId !== requestVersion.current) return;
      }
        ...
        >
          ...
          {state === "ready" && events.length === 0 ? (
            <section className={styles.dataState} role="status">
              ...
              <p>The latest successful snapshot contains no matching events.</p>
            </section>
          ) : null}
          {state === "ready" && events.length > 0 && view === "list" ? (
            <>
              <p
                ...
                aria-live="polite"
              >
                Showing {events.length} of {total} current events
              </p>
              <section
