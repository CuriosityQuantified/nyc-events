...
## Corpus Check
- 92 files · ~44,700 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 692 nodes · 1029 edges · 58 communities (40 shown, 18 thin omitted)
- Extraction: 97% EXTRACTED · 3% INFERRED · 0% AMBIGUOUS · INFERRED: 33 edges (avg confidence: 0.53)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `5d7bb04d`
...
- `TestNetworkEnforcement` --uses--> `SocrataError`  [INFERRED]
  backend/tests/test_ingestion.py → backend/app/socrata.py
- `TestResponseValidation` --uses--> `SocrataError`  [INFERRED]
  backend/tests/test_socrata.py → backend/app/socrata.py
...
Cohesion: 0.07
Nodes (49): async_sessionmaker, get_session_factory(), AsyncSession, Return a singleton async session factory., Dispose of the current engine. Use for testing or reconfiguration., reset_engine(), Event, A single NYC Parks event identified by its source guid. (+41 more)
...
Cohesion: 0.05
Nodes (43): Event SQLAlchemy model for NYC Parks events., _derive_borough(), _derive_registration(), _location_key(), _normalize_socrata_url(), _parse_categories(), _parse_coordinates(), _parse_date() (+35 more)

### Community 46 - "get_settings"
Cohesion: 0.07
Nodes (35): AsyncEngine, get_settings(), Application configuration via environment variables., Return a cached Settings instance., Settings loaded from environment variables., Settings, get_engine(), Async SQLAlchemy engine and session factory. (+27 more)
...
- **Why does `load_fixture()` connect `load_fixture` to `conftest.py`, `SocrataClient`, `socrata.py`?**
  _High betweenness centrality (0.032) - this node is a cross-community bridge._
- **Why does `Event` connect `conftest.py` to `load_fixture`, `socrata.py`, `get_settings`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Why does `get_settings()` connect `get_settings` to `SocrataClient`, `conftest.py`, `socrata.py`?**
