...
## Summary
- 800 nodes · 1302 edges · 70 communities (51 shown, 19 thin omitted)
...
Cohesion: 0.05
Nodes (51): AsyncClient, AsyncEngine, get_settings(), Application configuration via environment variables., Return a cached Settings instance., Settings loaded from environment variables., Settings, get_engine() (+43 more)
...
Cohesion: 0.05
Nodes (63): async_sessionmaker, CurrentEventSearch, get_current_event(), Any, The two bounded, read-only Event data operations used by the concierge., Validated search inputs with a hard result bound., Search only the latest current Snapshot in deterministic order., Retrieve one current Event by source guid; archival rows stay hidden. (+55 more)
...
- **Why does `sync_events()` connect `CurrentEvent` to `SocrataClient`, `ingest_rows`, `conftest.py`, `socrata.py`, `railway_release.py`?**
  _High betweenness centrality (0.050) - this node is a cross-community bridge._
- **Why does `load_fixture()` connect `load_fixture` to `test_contract.py`, `ingest_rows`, `test_events.py`, `SocrataClient`, `TestParseEvent`, `conftest.py`, `CurrentEvent`?**
  _High betweenness centrality (0.042) - this node is a cross-community bridge._
- **Why does `CurrentEvent` connect `CurrentEvent` to `ingest_rows`, `test_events.py`, `load_fixture`, `conftest.py`, `socrata.py`?**
