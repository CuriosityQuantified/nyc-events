...
Cohesion: 0.06
Nodes (32): ingest_rows(), Any, Parse raw Socrata rows and merge them into the database. This is the shared…, _build_validator(), _load_spec(), Any, requires_docker, Contract tests — validate API responses against the OpenAPI schema. (+24 more)
...
### Community 48 - "Event"
Cohesion: 0.16
Nodes (13): Event, Event SQLAlchemy model for NYC Parks events., A single NYC Parks event identified by its source guid., Base, SQLAlchemy model definitions., Base class for all SQLAlchemy models., AlwaysErrorTransport, Transport that always returns the given error status code. (+5 more)
...
- **Why does `Event` connect `Event` to `SocrataClient`, `ingest_rows`, `events.py`, `socrata.py`, `CredentialFilter`, `ingest_events`, `SocrataError`, `conftest.py`?**
  _High betweenness centrality (0.055) - this node is a cross-community bridge._
- **Why does `load_fixture()` connect `load_fixture` to `Event`, `conftest.py`, `SocrataClient`, `ingest_rows`?**
